...
/**
 * Premium dark-mode wallet picker. Mobile-first: full-width sheet on small
 * screens, centered card on >= sm. Touch targets are >= 44px (guideline §3.3).
 */
export function WalletModal({ open, onClose }: WalletModalProps): JSX.Element | null {
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="relative z-10 w-full max-w-md overflow-hidden rounded-t-3xl border border-white/10 bg-[#0d1228] shadow-[0_30px_120px_-20px_rgba(99,102,241,0.45)] sm:rounded-3xl"
      >
        <div className="flex items-start justify-between gap-4 border-b border-white/5 px-5 py-4 sm:px-6 sm:py-5">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="-m-2 inline-flex h-11 w-11 items-center justify-center rounded-full text-white/60 transition hover:bg-white/5 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
          >
            <svg
              viewBox="0 0 24 24"
              className="h-5 w-5"
        ...
        <ul className="flex flex-col gap-2 px-3 py-3 sm:px-4 sm:py-4">
          ...
          {connectors.map((connector) => {
            const isPending =
              status === "pending" && variables?.connector === connector;
            return (
        ...
        {error && (
          <div
            role="alert"
            className="mx-4 mb-4 rounded-xl border border-rose-400/30 bg-rose-500/10 px-3 py-2 text-xs text-rose-200 sm:mx-5"
          >
            {error.message}
          </div>
        )}
